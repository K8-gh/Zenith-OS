...
sudo apt-get install -y \
  ...
  arc-theme papirus-icon-theme gtk2-engines-murrine adwaita-icon-theme \
  ...
  mesa-utils mesa-vulkan-drivers xserver-xorg-video-intel xserver-xorg-video-nouveau vulkan-tools nvidia-detect \
...
# Flathub is enabled for optional applications offered by Zenith App Center.
...
printf '%s\n' 'GTK_THEME=Adwaita:dark' | sudo tee /etc/environment.d/90-zenith-dark-theme.conf >/dev/null 2>&1 || true
xfconf-query -c xsettings -p /Gtk/FontName -s 'Noto Sans 10' 2>/dev/null || true
xfconf-query -c xfwm4 -p /general/theme -s Daloa 2>/dev/null || xfconf-query -c xfwm4 -p /general/theme -s Default 2>/dev/null || true
